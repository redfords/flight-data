...
data = data.iloc[:, 1:]
data.to_sql('country', con = engine, if_exists = 'append', index_label = 'country_id', chunksize = 100)

# insert city data into MySQL
data = extract_from_csv('city.csv')
data = data.iloc[:, 1:]
data.to_sql('city', con = engine, if_exists = 'append', index_label = 'city_id', chunksize = 100)

# insert airline data into MySQL
data = extract_from_csv('airline.csv')
data = data.iloc[:, 1:]
data.to_sql('airline', con = engine, if_exists = 'append', index_label = 'airline_id', chunksize = 100)

# insert airport data into MySQL
data = extract_from_csv('airport.csv')
data = data.iloc[:, 1:]
data.to_sql('airport', con = engine, if_exists = 'append', index_label = 'airport_id', chunksize = 100)
